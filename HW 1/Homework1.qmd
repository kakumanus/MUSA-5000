...
```{r untransformed-histograms}
#| echo: false

# Reshape data to long format manually
...
plot_data <- data.frame(
  value = c(
    ...
    regression_data$LNPCTBACHMOR,
    regression_data$LNNBELPOV100,
    regression_data$LNPCTVACANT,
    regression_data$LNPCTSINGLES
  ),
  variable = rep(
    c(
      "Log Median House Value",
      "Log Percent with Bachelor's Degree or More",
      "Log Number Below Poverty",
      "Log Percent of Vacant Homes",
      "Log Percent of Single Family Homes"
    ),
    each = nrow(regression_data)
  )
)
...
```{r}
#| echo: false
Regression_shpData <- st_read("./Lecture 1 - RegressionData.shp", quiet = TRUE)
